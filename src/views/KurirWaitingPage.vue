<template>
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      <ion-title class="ion-text-left"> Pesanan Baru </ion-title>
            ...
            <ion-label>
              ...
              <div class="order-details-container">
                ...
                <div class="order-details">
                  ...
                  <p class="value" v-if="order.total_pembayaran !== undefined">Rp{{ order.total_pembayaran.toLocaleString('id-ID') }}</p>  
                </div>
                <div class="order-details">
                ...
                <div class="button-container">
                  <template v-if="order.status === 'Menunggu'">
                    <ion-button @click="updateOrderStatus(order.id_order, 'Dikonfirmasi')">Konfirmasi</ion-button>
                    <ion-button @click="updateOrderStatus(order.id_order, 'Pesanan Batal')">Batalkan</ion-button>
                  </template>
                  <template v-else-if="order.status === 'Dikonfirmasi'">
                    <ion-button @click="updateOrderStatus(order.id_order, 'Dalam Pengiriman'), navigateToLocationPage(order.alamat)">Kirim</ion-button>
                    <ion-button @click="updateOrderStatus(order.id_order, 'Pesanan Batal')">Batalkan</ion-button>
                  </template>
                </div>
              </div>
...
export default {
  ...
  computed: {
    filteredOrders() {
      return this.orders
        .filter(order => order.status === 'Menunggu' || order.status === 'Dikonfirmasi')
